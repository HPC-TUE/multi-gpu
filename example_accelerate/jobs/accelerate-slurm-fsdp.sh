#!/bin/bash
#SBATCH --job-name=hf_accelerate_train_fsdp
#SBATCH --time=0:30:00
#SBATCH --partition=gpu_h100
#SBATCH --nodes=2
#SBATCH --gpus-per-node=4
#SBATCH --tasks-per-node=1 # crucial - only 1 task per dist per node!
...
accelerate launch --config_file ./example_accelerate/configs/deepspeed_config.yaml \
    ...
    ./example_accelerate/accelerate_training.py \
    --project_name="multi_slurm_test_accelerate" \
    --entity="AI_team_SCC_TUe" \
    --run_name="test_accelerate_multinode_fsdp_example" \
    $CHECKPOINT_ARG "$@"
